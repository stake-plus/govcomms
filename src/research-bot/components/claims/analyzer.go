...
func (a *Analyzer) VerifyClaims(ctx context.Context, claims []Claim) ([]VerificationResult, error) {
	...
	log.Printf("Waiting 30 seconds before starting verification...")
	select {
	case <-ctx.Done():
	...
	for i := 0; i < len(claims); i++ {
		...
		// Wait 30 seconds between each claim to avoid rate limiting
		if i < len(claims)-1 {
			log.Printf("Waiting 30 seconds before next claim...")
			select {
			case <-ctx.Done():
				return results, ctx.Err()
			case <-time.After(30 * time.Second):
