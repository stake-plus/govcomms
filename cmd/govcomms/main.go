...
import (
    ...
    aiqaconfig "github.com/stake-plus/govcomms/src/ai-qa/config"
    aiqabot "github.com/stake-plus/govcomms/src/ai-qa/bot"
    fbbot "github.com/stake-plus/govcomms/src/feedback/bot"
    fbconfig "github.com/stake-plus/govcomms/src/feedback/config"
    fbdata "github.com/stake-plus/govcomms/src/feedback/data"
    rbconfig "github.com/stake-plus/govcomms/src/research-bot/config"
...
func main() {
    ...
    if *enableQA {
        cfg := aiqaconfig.Load(db)
        qa, err = aiqabot.New(&cfg, db)
        if err != nil { log.Fatalf("qa bot: %v", err) }
        if err := qa.Start(); err != nil { log.Fatalf("qa start: %v", err) }
        log.Printf("AI Q&A started")
    }

    if *enableResearch {
        cfg := rbconfig.Load(db)
        research, err = rbbot.New(&cfg, db)
        if err != nil { log.Fatalf("research bot: %v", err) }
        if err := research.Start(); err != nil { log.Fatalf("research start: %v", err) }
        log.Printf("Research bot started")
    }

    if *enableFeedback {
        cfg := fbconfig.Load(db)
        rdb := fbdata.MustRedis(cfg.RedisURL)
        feedback, err = fbbot.New(&cfg, db, rdb)
        if err != nil { log.Fatalf("feedback bot: %v", err) }
        if err := feedback.Start(); err != nil { log.Fatalf("feedback start: %v", err) }
    ...
    if qa != nil { qa.Stop() }
    if research != nil { research.Stop() }
    if feedback != nil { feedback.Stop() }
}
